...
namespace LtAmpDotNet.Lib.Extensions
{
    public static class MessageExtensions
    {
        public static byte[][] ToUsbMessage(this IMessage message, int packetLength = 65)
        {
            ...
            List<byte[]> chunks = [.. data.Split(packetLength)];
            ...
            for (int i = 0; i < chunks.Count; i++)
            {
                ...
                packets[i][2] = Convert.ToByte(chunks[i].Length);
        ...
        {
            return arr.Select((s, i) => arr.Skip(i * chunkSize).Take(chunkSize).ToArray()).Where(a => a.Length != 0).ToArray();
        }
    }
}