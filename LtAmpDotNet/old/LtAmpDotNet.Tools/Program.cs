...
namespace LtAmpDotNet.Tools
{
    internal class Program
    {
        ...
        private static void Main(string[] args) => DecodeTestStrings("C:\\Users\\brent\\OneDrive\\Projects\\FenderTone\\bypass.txt", "C:\\Users\\brent\\OneDrive\\Projects\\FenderTone\\bypass2.txt");
        //=> OutputEffectsInCsv("C:\\Users\\Brent Maxwell\\OneDrive\\Projects\\FenderTone\\dspUnits.csv");//ExtractData(//    "C:\\Users\\Brent Maxwell\\OneDrive\\Projects\\FenderTone\\fenderTone\\Fender Tone LT Desktop.exe",//    "C:\\Users\\Brent Maxwell\\OneDrive\\Projects\\FenderTone\\fenderTone\\extracts");//amp = new Lib.LtAmplifier();//amp.MessageReceived += Amp_MessageReceived;//amp.MessageSent += Amp_MessageSent;////amp.DeviceConnected += Amp_DeviceConnected;//amp.Open();//List<Preset>? presets = JsonConvert.DeserializeObject<List<Preset>>(File.ReadAllText(Path.Join(Environment.CurrentDirectory, "JsonDefinitions", "mustang", "default_presets.json")));//foreach (var preset in presets!)//{//    Console.WriteLine(preset);//}//var input = args[0];//var output = args[1];//DecodeTestStrings(input, output);//Console.Read();//foreach(var item in LtAmplifier.DspUnitDefinitions.OrderBy(x => x.Info.SubCategory))//{//    Console.WriteLine($"{item.Info.SubCategory}\t{item.FenderId}\t//{item.Info.DisplayName}");//}

        private static void Amp_MessageSent(FenderMessageLT message) => Console.WriteLine($">> {message}");

        private static void Amp_DeviceConnected(object sender, EventArgs e) => amp.RenamePresetAt(4, "DOOKIE  CLEAN");

        private static void Amp_MessageReceived(FenderMessageLT message) => Console.WriteLine($"<< {message}");
