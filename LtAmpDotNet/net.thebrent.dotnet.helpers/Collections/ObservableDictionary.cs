...
namespace net.thebrent.dotnet.helpers.Collections
{
    public class ObservableDictionary<TKey, TValue> : IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, INotifyCollectionChanged, INotifyPropertyChanged where TKey : notnull
    {
        ...
        {
            Add(item: new KeyValuePair<TKey, TValue>(key, value));
        }
        ...
        public void Clear()
        {
            if (Items.Any())
            {
                List<KeyValuePair<TKey, TValue>> removedItems = [.. Items];
                Items.Clear();
                OnCollectionChanged(
        ...
        [SuppressMessage("Performance", "CA1841:Prefer Dictionary.Contains methods", Justification = "This is the contains keys method")]
        ...
        {
            Items.CopyTo(array: array, arrayIndex: arrayIndex);
        }
        ...
        private bool InsertObject(TKey key, TValue value, AppendMode appendMode)
        {
            return InsertObject(key, value, appendMode, out _);
        }
